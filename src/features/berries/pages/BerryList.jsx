...
/**
 * BerryList Component
 * Displays paginated list of Pokémon berries with search functionality
 */
const BerryList = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto pb-20 px-4 relative">
      ...
      <div className="flex flex-col md:flex-row justify-between items-end md:items-center mb-10 gap-4 mt-6">
        <div>
          ...
          <p className="text-gray-500 dark:text-gray-400 mt-2 transition-colors">
            Explore the complete list of Pokémon berries.
          </p>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6">
          {berriesToDisplay.map((berry) => {
            ...
            return (
              <div
                key={berry.name}
                onClick={() => setSelectedBerryModal(berry.name)}
                className="bg-white dark:bg-gray-800 rounded-2xl p-4 shadow-sm hover:shadow-xl dark:hover:shadow-lg dark:hover:shadow-emerald-500/10 hover:-translate-y-1 transition-all duration-300 cursor-pointer border border-gray-100 dark:border-gray-700 group flex flex-col items-center"
              >
                <div className="bg-emerald-50/50 dark:bg-emerald-900/20 rounded-xl p-4 mb-4 flex justify-center relative w-full min-h-[120px] items-center transition-colors">
                  <span className="absolute top-2 left-3 text-xs font-bold text-emerald-600/50 dark:text-emerald-400/50">
                    #{id.padStart(3, "0")}
                  </span>
                  <img
                    src={imageUrl}
      ...
      {!isSearching && infiniteQuery.status === "success" && (
        <div className="mt-12 flex justify-center">
          <button
            ...
            className="bg-emerald-500 hover:bg-emerald-600 dark:bg-emerald-600 dark:hover:bg-emerald-700 text-white font-semibold px-8 py-3 rounded-full shadow-md transition-all flex items-center gap-2 disabled:opacity-50"
          >
      ...
      {showScrollTop && (
        <button
          onClick={scrollToTop}
          className="fixed bottom-6 right-6 p-4 bg-emerald-500 text-white rounded-full shadow-lg shadow-emerald-500/40 hover:bg-emerald-600 hover:-translate-y-1 transition-all z-[40] flex items-center justify-center group"
