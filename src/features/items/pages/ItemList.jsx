...
const ItemList = () => {
  ...
  useEffect(() => {
    const observer = new IntersectionObserver(([entry]) => setShowScrollTop(!entry.isIntersecting), { rootMargin: "-300px 0px 0px 0px" });
  ...
  return (
    <div className="max-w-7xl mx-auto pb-20 px-4 relative">
      ...
      {selectedItemModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 transition-all" onClick={() => setSelectedItemModal(null)}>
      ...
      <div className="flex flex-col md:flex-row justify-between items-end md:items-center mb-10 gap-4 mt-6">
        <div>
          ...
          <p className="text-gray-500 dark:text-gray-400 mt-2 transition-colors">Explore the complete list of Pokémon items.</p>
        </div>
        <form onSubmit={handleSearch} className="relative w-full md:w-80">
          ...
          {searchTerm && (
            <button type="button" onClick={() => setSearchTerm("")} className="absolute right-3 top-2.5 text-gray-400 hover:text-amber-500 dark:hover:text-amber-400 transition-colors">
              <X className="w-5 h-5" />
            </button>
          )}
        </form>
      ...
      {isSearching && itemsToDisplay.length === 0 && <div className="text-center p-10 text-gray-400 dark:text-gray-500 font-medium transition-colors">No items found matching "{searchTerm}"</div>}
      ...
      {!isLoading && !isError && (
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6">
          {itemsToDisplay.map((item) => {
            ...
            return (
              <div key={item.name} onClick={() => setSelectedItemModal(item.name)} className="bg-white dark:bg-gray-800 rounded-2xl p-4 shadow-sm hover:shadow-xl dark:hover:shadow-lg dark:hover:shadow-amber-500/10 hover:-translate-y-1 transition-all duration-300 cursor-pointer border border-gray-100 dark:border-gray-700 group flex flex-col items-center">
                ...
                <h3 className="text-center font-bold text-gray-700 dark:text-gray-100 capitalize text-lg transition-colors">{item.name.replace(/-/g, " ")}</h3>
              </div>
            );
          })}
        </div>
      ...
      {!isSearching && infiniteQuery.status === "success" && (
        <div className="mt-12 flex justify-center">
          <button onClick={() => infiniteQuery.fetchNextPage()} disabled={!infiniteQuery.hasNextPage || infiniteQuery.isFetchingNextPage} className="bg-amber-500 hover:bg-amber-600 dark:bg-amber-600 dark:hover:bg-amber-700 text-white font-semibold px-8 py-3 rounded-full shadow-md transition-all flex items-center gap-2 disabled:opacity-50">
            {infiniteQuery.isFetchingNextPage ? <><Loader2 className="animate-spin w-5 h-5" /> Loading...</> : infiniteQuery.hasNextPage ? "Load More Items" : "End of List"}
          </button>
        </div>
      )}

      {showScrollTop && (
        <button onClick={scrollToTop} className="fixed bottom-6 right-6 p-4 bg-amber-500 text-white rounded-full shadow-lg shadow-amber-500/40 hover:bg-amber-600 hover:-translate-y-1 transition-all z-[40] flex items-center justify-center group" aria-label="Scroll to top">
