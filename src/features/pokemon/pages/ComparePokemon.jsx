...
const TYPE_COLORS = {
  grass: "#22c55e", fire: "#ef4444", water: "#3b82f6", bug: "#65a30d",
  normal: "#9ca3af", poison: "#a855f7", electric: "#eab308", ground: "#d97706",
  fairy: "#f472b6", fighting: "#c2410c", psychic: "#db2777", rock: "#57534e",
  ghost: "#3730a3", ice: "#67e8f9", dragon: "#4f46e5", dark: "#1f2937",
  steel: "#6b7280", flying: "#38bdf8",
};
  ...
  if (active && payload && payload.length) {
    return (
      <div className="bg-white/90 dark:bg-gray-800/90 backdrop-blur-md p-3 rounded-xl border border-gray-200 dark:border-gray-700 shadow-xl">
        ...
        {payload.map((entry) => (
          <p key={entry.name} className="text-sm font-bold flex justify-between gap-4 capitalize" style={{ color: entry.color }}>
  ...
  return (
    <div className="relative w-full" ref={wrapperRef}>
      <div className="bg-white dark:bg-gray-800 p-2 rounded-2xl shadow-inner border-2 border-gray-100 dark:border-gray-700 flex items-center gap-2">
        <img src="https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/items/poke-radar.png" alt="Search" className="w-6 h-6 object-contain ml-2 animate-pulse" />
        <input
          type="text"
        ...
        />
        {searchTerm && (
          <button onClick={() => setSearchTerm("")} className="mr-2 text-gray-400 hover:text-red-500 transition-colors cursor-pointer">
            <X size={18} />
          </button>
        )}
      </div>
        ...
        <div className="absolute top-full left-0 right-0 mt-2 bg-white dark:bg-gray-800 border-2 border-gray-100 dark:border-gray-700 rounded-2xl shadow-xl z-50 overflow-hidden animate-in fade-in slide-in-from-top-2">
          {filtered.map((p) => {
            ...
            return (
              ...
              >
                <img src={`https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/${id}.png`} alt={p.name} className="w-10 h-10 object-contain group-hover:scale-110 transition-transform rendering-pixelated" />
                <span className="font-bold capitalize text-gray-700 dark:text-gray-200">{p.name.replace("-", " ")}</span>
              </button>
            );
          })}
        </div>
...
const ComparePokemon = () => {
  ...
  const { data: p1 } = useQuery({ queryKey: ["pokemon", pokemon1Name], queryFn: () => fetchPokemonDetail(pokemon1Name), enabled: !!pokemon1Name });
  const { data: p2 } = useQuery({ queryKey: ["pokemon", pokemon2Name], queryFn: () => fetchPokemonDetail(pokemon2Name), enabled: !!pokemon2Name });
  ...
  const renderCard = (pData, onRemove) => {
    ...
    return (
      <div className="relative flex flex-col items-center bg-white dark:bg-gray-800 rounded-[2rem] p-6 shadow-xl border-4 border-transparent hover:border-gray-200 dark:hover:border-gray-700 transition-all duration-500 w-full max-w-sm overflow-hidden group">
        <div className={`absolute top-0 left-0 w-full h-32 ${typeColorClass} opacity-20 dark:opacity-40 transition-colors`}></div>
        <button onClick={onRemove} className="absolute top-4 right-4 z-10 bg-white/50 hover:bg-red-500 hover:text-white rounded-full p-1.5 backdrop-blur-sm transition-colors text-gray-500 cursor-pointer">
          <X size={16} />
        </button>
        <img src={imgUrl} alt={pData.name} className="w-40 h-40 object-contain z-10 drop-shadow-2xl group-hover:scale-110 transition-transform duration-500" />
        <h2 className="text-2xl font-black capitalize text-gray-800 dark:text-white mt-4 z-10">{pData.name.replace("-", " ")}</h2>
        <div className="flex gap-2 mt-2 z-10">
          {pData.types.map(t => {
            const badgeColor = TYPE_BG_CLASSES[t.type.name] || "bg-gray-500";
            return (
              <span key={t.type.name} className={`px-3 py-1 ${badgeColor} text-white rounded-full text-xs font-bold capitalize shadow-sm flex items-center gap-1.5`}>
                <img src={`https://raw.githubusercontent.com/duiker101/pokemon-type-svg-icons/master/icons/${t.type.name}.svg`} alt={t.type.name} className="w-3.5 h-3.5 object-contain drop-shadow-sm" />
                {t.type.name}
              </span>
            );
          })}
  ...
  return (
    <div className="max-w-6xl mx-auto pb-20 px-4 mt-6">
      ...
      <div className="flex items-center justify-center gap-4 mb-10">
        <img src="https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/items/vs-seeker.png" alt="VS" className="w-10 h-10 object-contain drop-shadow-md animate-bounce" />
        ...
        <div className="bg-white dark:bg-gray-800 rounded-[3rem] p-6 md:p-10 shadow-2xl border border-gray-100 dark:border-gray-700 animate-in zoom-in-95 duration-700 flex flex-col lg:flex-row items-center gap-10">
          ...
          <div className="w-full lg:w-1/2 h-[350px] md:h-[450px]">
            <ResponsiveContainer width="100%" height="100%">
              <RadarChart cx="50%" cy="50%" outerRadius="70%" data={radarData}>
                ...
                {p1 && <Radar name={p1.name.replace("-", " ")} dataKey={p1.name} stroke={c1} fill={c1} fillOpacity={0.4} strokeWidth={3} />}
                {p2 && <Radar name={p2.name.replace("-", " ")} dataKey={p2.name} stroke={c2} fill={c2} fillOpacity={0.4} strokeWidth={3} />}
              </RadarChart>
            </ResponsiveContainer>
          </div>
          ...
          <div className="w-full lg:w-1/2 flex flex-col gap-3">
            <h3 className="text-xl font-black text-center text-gray-800 dark:text-white mb-4 flex items-center justify-center gap-2">
              <img src="https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/items/data-card-01.png" alt="Data" className="w-6 h-6 object-contain" />
              Stat Breakdown
            </h3>
            ...
            {basePokemon.stats.map((stat, idx) => {
              ...
              return (
                <div key={statName} className="flex items-center justify-between bg-gray-50 dark:bg-gray-700/50 p-3 md:p-4 rounded-2xl hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors cursor-default">
                  <div 
            ...
            <div className="flex items-center justify-between bg-gray-800 dark:bg-gray-900 p-4 rounded-2xl text-white mt-2 shadow-inner border border-gray-700">
              <div 
                ...
                style={p1 && p2 && p1.stats.reduce((a,b)=>a+b.base_stat,0) > p2.stats.reduce((a,b)=>a+b.base_stat,0) ? { color: c1 } : {}}
              >
              ...
              <div 
                className="w-16 text-center font-black text-xl transition-colors"
                style={p1 && p2 && p2.stats.reduce((a,b)=>a+b.base_stat,0) > p1.stats.reduce((a,b)=>a+b.base_stat,0) ? { color: c2 } : {}}
              >
